...
- A Homebrew tap (`brew install agentguard`).
- `install.sh` (`curl | sh`).
- An **npm shim package** (`agentguard`) that downloads the platform binary on
  postinstall so the JS crowd can `npx agentguard scan`.
